...
dat = read.csv("hourlydat/city_pol_h.csv", stringsAsFactors = F, sep = ";")[1:500000,] %>%
  ...
  # one-year shijiazhuang data from 2018.02 - 2019.02
  filter(as.Date(time) > '2018-01-31',
         as.Date(time) < '2019-02-01',
         substr(station_code, 1, 5) == 'G1301',
...
dist_loc2 <- stations_loc %>%
  filter(substr(station_code, 1, 6) == 'G13010',
         station_code != 'G130104') %>% 
  select(lon, lat) %>%
  dist() %>% as.matrix()
```
...
for (s in c('Spring', 'Summer', 'Fall', 'Winter')) {
  ...
  name = colnames(dat)
  #Ymat = log(t(dat))
  Ymat <- matrix(log(as.numeric(t(dat))), 8, nrow(dat))
  ...
  print(s)
  cat('$station (s)$ & ', paste(paste0('s', 1:8), collapse = ' & '), "\\\\ \n")
  ...
  pred[[j]] <- predKriging(Ymat, dist_loc2, 4)
  j <- j + 1
}
...
save(pred, file = 'pred-sjz-ori.RData')
```
