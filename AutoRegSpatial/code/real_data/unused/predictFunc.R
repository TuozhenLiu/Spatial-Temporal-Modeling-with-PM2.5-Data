...
predict.typeII<-function(Y_train, rhos_train, dist_train, dist_pred, thetaE, thetaX, mu)
{
  betaE = thetaE[1]; sig2E = thetaE[2]
  alpha = thetaX[1]; betaX = thetaX[2]; sig2X = thetaX[3]
  Time = ncol(Y_train)
  eps_train = Y_train[,-1] - rhos_train*Y_train[,-Time]-mu
  ...
  if (is.null(dim(dist_pred)))
    Y_pred = sum(eps_pred[, 1:(ncol+1)]*c(rhosk_pred, 1)[1:(ncol+1)]) + rhos_inv_pred*mu
  else
    Y_pred = rowSums(eps_pred[, 1:(ncol+1)]*cbind(rhosk_pred, 1)[,1:(ncol+1)]) + rhos_inv_pred*mu
  return(Y_pred)
}
