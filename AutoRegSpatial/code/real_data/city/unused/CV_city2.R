...
for (begin_ind in c(1))
{
  ...
  for (est_win in c(60))
  {
    ...
    {
      pred_inds = sort(sample(1:nrow(loc1), 50))#[-c(9,49)]
      ...
      Yt_pred1[i, t-begin_ind+1] = predict.typeI(Y_train, rhos_est, mus_est, loc_train = loc1[train_ind,], loc_pred = loc1[pred_ind,],
                                                 thetaE, thetaX, thetaMu, cov.type = cov.type)
