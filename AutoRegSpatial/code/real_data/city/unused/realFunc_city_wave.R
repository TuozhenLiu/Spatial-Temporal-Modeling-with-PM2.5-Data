...
# wave covariance model
cov.wave <- function(x, phi, sig2)
{
  phis = phi/(6.5*pi*x)
  cov_wave = sig2*phis*sin(1/phis)
  diag(cov_wave) = sig2
  return(cov_wave)
}
...
simu.rho <- function(phi, sig2, N, alpha, dist_loc2)
{
...
simu.rho.rep <- function(phi, sig2, N, R, alpha, dist_loc2)
{
  cov_X = cov.wave(dist_loc2, phi, sig2)
...
simu.mu.rep <- function(phi, sig2, N, R, mu, dist_loc2)
{
  cov_X = cov.wave(dist_loc2, phi, sig2)
...
simu.Y <- function(phi, sigy2, N, Time, rhos, dist_loc2, mu)
{
  #### simulate covariance of epsilon
  cov_e = cov.wave(dist_loc2, phi, sigy2)
...
simu.Y.rep <- function(phi, sigy2, N, R, Time, rhos, dist_loc2, mus)
{
  #### simulate covariance of epsilon
  cov_e = cov.wave(dist_loc2, phi, sigy2)
...
lse.step <- function(sig_hat, phi, sigy2, dist_loc2)
{
  ...
  ### hessian matrix
  sec_phi = -sigy2/phi^2*cos_phis - sigy2/phis/phi*sin_phis
