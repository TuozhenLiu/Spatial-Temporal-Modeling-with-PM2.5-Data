  ...
  for (r in 1:Nrep)
  {
    ...
    for (begin_ind in c(1))
    {
      ...
      for (t in begin_ind:end_ind)
      {
        ...
        Yt_pred2[pred_ind, t-begin_ind+1] = predict.typeII(Y_train, rhos_est, dist_train, dist_pred, 
                                                           thetaE, thetaX, mu = mu_est)
        Yt_pred1[pred_ind, t-begin_ind+1] = predict.typeI(Y_train, rhos_est, loc_train = loc1[train_ind,], loc_pred = loc1[pred_ind,],
                                                          thetaE, thetaX, mu = mu_est)
    ...
    perf_r2[r,] = c(1-(krig1_rmse/sse)^2, 1-(krig2_rmse/sse)^2, 
                    1- (pred1_rmse/sse)^2, 1- (pred2_rmse/sse)^2)
