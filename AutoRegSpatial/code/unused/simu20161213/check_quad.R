...
object.func<-function(beta, sig2, sig_hat)
{
  cov_X = cov.quad(dist_loc2, beta, sig2)
...
lse.X<-function(Y, dist_loc2, theta_true) # Y here is X-alpha, which is centered
{
  ...
  {
    #cat(mean(abs(del)), " ", theta, "\n")
    del = lse.step(sig_hat, beta = theta[1], sigy2 = 1, dist_loc2)
    theta = theta - del*0.5
    if (any(theta<0|theta>10))
  ...
  return(list(abs(theta), sqrt(est_cov), CI))
}
...
#     iter = iter+1
#   }
#   est_cov = infer.cov(sig_hat, beta = theta[1], sigy2 = 1, dist_loc2, Time = 1)
