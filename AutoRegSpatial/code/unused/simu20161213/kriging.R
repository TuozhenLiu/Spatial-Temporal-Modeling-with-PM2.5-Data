...
betaE = 2; sig2E = 1
betaX = 3; sig2X = 0.5; alpha = 0.5
thetaX_true = c(alpha, betaX, sig2X)
theta = c(alpha, betaX, sig2X, betaE, sig2E)
...
for (i in 1:length(Ns))
{
  ...
  for (t in 1:length(Times))
  {
    ...
    theta_est = matrix(0, nrow = 5, ncol = Nrep)
    ...
    for (r in 1:Nrep)
    {
      ...
      Y = simu.Y(beta1 = betaE, sigy2 = sig2E, N = N, Time = Time, rhos = rhos, dist_loc2, mu = 0)
      ...
      Yt_pred2_true = krigingMu.typeII(Y_train, rhos_train, dist_train, dist_pred, 
                                       thetaE = c(betaE, sig2E), thetaX = c(alpha, betaX, sig2X), mu = 0)
      
      #Yt_pred1_true = kriging.typeI(Y_train, rhos_train, dist_train, dist_pred, 
      #                             thetaE = c(betaE, sig2E), thetaX = c(alpha, betaX, sig2X))
      Yt_pred1_true = krigingMu.typeI(Y_train, rhos_train, loc_train = loc[train_ind,], loc_pred = loc[pred_ind,],
                                      thetaE = c(betaE, sig2E), thetaX = c(alpha, betaX, sig2X), mu = 0)
      
      rhos_est = estRho(Y_train)
      ...
      theta_est[,r] = c(thetaX, thetaE)
      
      
      Yt_pred2 = krigingMu.typeII(Y_train, rhos_est, dist_train, dist_pred, 
                                  thetaE, thetaX, mu = 0)
      
      Yt_pred1 = krigingMu.typeI(Y_train, rhos_est, loc_train = loc[train_ind,], loc_pred = loc[pred_ind,],
                                 thetaE, thetaX, mu = 0)
      
      res0[,r] = c(mean((Yt_pred1_true - Y[pred_ind,1])),
                   mean((Yt_pred2_true - Y[pred_ind,1])),
                   mean((Yt_pred1 - Y[pred_ind,1])),
                   mean((Yt_pred2 - Y[pred_ind,1])))
      res[,r] = c(mean((Yt_pred1_true - Y[pred_ind,1])^2),
                  mean((Yt_pred2_true - Y[pred_ind,1])^2),
                  mean((Yt_pred1 - Y[pred_ind,1])^2),
                  mean((Yt_pred2 - Y[pred_ind,1])^2))
      cat(r, "\r")
    }
    estRes[[t]] = list(res0, res, theta_est, thetaX_true_est)
    cat("\n N: ", N, " Time:", Time, "\n",
        "Kriging Bias: ",rowMeans(res0),"\n", 
        "Kriging MSE: ", sqrt(rowMeans(res)),"\n",
