# This is for mixed simulation and estimation.
...
# set.seed(1234)
# loc = simu.loc(100, cov.type = cov.type)
...
for (i in 1:length(Ns))
{
  ...
  for (t in 1:length(Times))
  {
    ...
    for (r in 1:Nrep)
    {
      loc = simu.loc(N, cov.type = "quad")
      dist_loc2 = as.matrix(dist(loc))
      rhos = simu.rho(beta = betaX, sig2 = sig2X, N, alpha = alpha, dist_loc2, cov.type = cov.type1)
      mus = simu.mu(beta = betaMu, sig2 = sig2Mu, N, mu = mu, dist_loc2, cov.type = cov.type1)
      Y = simu.Y(beta = betaE, sigy2 = sig2E, N = N, Time = Time, rhos = rhos, 
                 dist_loc2, mu = mus, cov.type = cov.type2)
      ...
      thetaE_list = lse.theta(Y_train, dist_train, rhos = rhos_est, mu = mus_est, 
                              theta_true = c(betaE, sig2E), cov.type = cov.type2)
      ...
      thetaX_list = estThetaRho(rhos_est, dist_train, theta_true = c(alpha, betaX, sig2X), cov.type = cov.type1)
      thetaX = thetaX_list[[1]]
      thetaXCI[,r] = thetaX_list[[3]]
      thetaX_sd[,r] = (thetaX_list[[2]])
      
      thetaMu_list = estThetaMu(mus_est, dist_train, theta_true = c(mu, betaMu, sig2Mu), cov.type = cov.type1)
      ...
      thetaE_true_list = lse.theta(Y_train, dist_train, rhos = rhos_train, mu = mus_train, theta_true = c(betaE, sig2E), cov.type = cov.type2)
      thetaE_true = thetaE_true_list[[1]]
      thetaE_trueCI[,r] = thetaE_true_list[[3]]
      thetaE_true_sd[,r] = (thetaE_true_list[[2]])
      
      thetaX_true_list = estThetaRho(rhos_train, dist_train, theta_true = c(alpha, betaX, sig2X), cov.type = cov.type1)
      thetaX_true = thetaX_true_list[[1]]
      thetaX_trueCI[,r] = thetaX_true_list[[3]]
      thetaX_true_sd[,r] = (thetaX_true_list[[2]])
      
      thetaMu_true_list = estThetaMu(mus_train, dist_train, theta_true = c(mu, betaMu, sig2Mu), cov.type = cov.type1)
