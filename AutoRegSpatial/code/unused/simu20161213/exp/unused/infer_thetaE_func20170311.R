...
lse.X<-function(Y, dist_loc2, infer = T, theta_true, cov.type = "exp", p.value = T) # Y here is X-alpha, which is centered
{
  Time = ncol(Y)
  if(is.null(Time))
    Time = 1
...
lse.theta<-function(Y, dist_loc2, rhos, mu, infer = T, theta_true = c(1,1), cov.type = "exp", p.value = F)
{
  ...
  # 
  # 
  # sig2_hat = var(as.vector(Y))#mean(Y^2)
  # 
  # theta = c(0.5) # the initial value
  # iter = 1; del = 1
  # while(mean(abs(del))>10^-3&iter<1000)
  # {
  #   #cat(mean(abs(del)), " ", theta, "\n")
  #   del = lse.step(sig_hat, beta = theta, sigy2 = theta_true[2], dist_loc2) # each step
  #   theta = theta - del*0.5
  ...
  #   return(c(theta, sig2_hat))
  # 
  # est_cov = infer.cov(sig_hat, beta = theta[1], sigy2 = theta_true[2], dist_loc2, Time)
  # CI = (theta-1.96*sqrt((est_cov))<theta_true[1])&(theta_true[1]<theta+1.96*sqrt((est_cov)))
  # 
  # ### for sigma inference
  # if (cov.type == "quad")
