...
for (k in 1:length(Ns))
{
  ...
  for (tt in 1:length(Times))
  {
    ...
    for (r in 1:Nrep)
    {
      loc1 = data.frame(simu.loc(N, cov.type = "quad"))
      ...
      Ymat = simu.Y(beta = betaE, sigy2 = sig2E, N = N, Time = Time, rhos = rhos, dist_loc2, mu = mus, cov.type = cov.type2)
      ...
      for (begin_ind in c(1))
      {
        ...
        (thetaMu = estThetaMu(mus_est, dist_train, infer = F, cov.type = cov.type1))
        (thetaX = estThetaRho(rhos_est, dist_train, infer = F, cov.type = cov.type1))
        (thetaE = lse.theta(Y_train, dist_train, rhos = rhos_est, mu = mus_est, infer = F, cov.type = cov.type2))
        ...
        for (t in begin_ind:end_ind)
        {
          ...
          Yt_pred2[pred_ind, t-begin_ind+1] = predict.typeII(Y_train, rhos_est, mus_est, dist_train, dist_pred, 
                                                             thetaE, thetaX, thetaMu, cov.type = cov.type, mixed = T)
