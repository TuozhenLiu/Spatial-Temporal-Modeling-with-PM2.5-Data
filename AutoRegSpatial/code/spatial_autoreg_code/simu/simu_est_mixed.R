  ...
  for (t in 1:2)
  {
    ...
    for (r in 1:Nrep)
    {
      ...
      estMu_list = estThetaMu(mus_est, dist_loc2, infer = T, 
                              theta_true = c(mu, betaMu, sig2Mu), cov.type1, est_sig = F)
      estrho_list = estThetaRho(rhos_est, dist_loc2, infer = T, 
                                theta_true = c(alpha, betaX, sig2X), cov.type1, est_sig = F)
      
      estE_list = lse.theta(Y, dist_loc2, rhos = as.vector(rhos_est), mu = as.vector(mus_est), 
                            theta_true = c(betaE, sig2E), cov.type = cov.type2, est_sig = F)
      ...
      estMu_list = estThetaMu(mus, dist_loc2, infer = T, 
                              theta_true = c(mu, betaMu, sig2Mu), cov.type = cov.type1, est_sig = F)
      estrho_list = estThetaRho(rhos, dist_loc2, infer = T, 
                                theta_true = c(alpha, betaX, sig2X), cov.type = cov.type1, est_sig = F)
